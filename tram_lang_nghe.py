...
# ==========================================
# 1. LÁ CHẮN BẢO VỆ BỘ NHỚ (CHỐNG LỖI KEYERROR TRÊN CLOUD)
# ==========================================
def khoi_tao_bien_he_thong():
...
khoi_tao_bien_he_thong() # GỌI NGAY TỪ ĐẦU ĐỂ BẢO VỆ

# Khởi tạo màu Theme
theme_map = {"Xanh Zalo": "#0068ff", "Tím Tinh Tế": "#6366f1", "Xanh Lá Tươi": "#10b981", "Đen Huyền Bí": "#1f2937", "Đỏ Áp Lực": "#e53935"}
main_color = theme_map.get(st.session_state.get('theme_color', 'Xanh Zalo'), "#0068ff")

# ==========================================
# CẤU HÌNH HỆ THỐNG & CSS ÉP KỊCH TRẦN 
# ==========================================
...
st.markdown(f"""
    ...
    [data-testid="column"]:nth-of-type(1) div[data-testid="stButton"] button {{
        ...
        padding: 15px 0 !important; width: 100% !important; font-size: 16px !important;
    }}
    ...
    .zalo-list-btn button {{
        ...
        border-radius: 0px !important; padding: 10px 10px !important; text-align: left !important;
        justify-content: flex-start !important; color: #111 !important; margin-top: -1px !important; 
    }}
    .zalo-list-btn button p {{ margin: 0 !important; line-height: 1.4 !important; font-size: 14px !important; }}
...
# ==========================================
# GIAO DIỆN ĐĂNG NHẬP (Đã Fix Lỗi Kẹt Cache)
# ==========================================
def kiem_tra_dang_nhap(role_can_thiet=None):
    ...
    if not current_user:
        ...
        with c2:
            u = st.text_input("Tài khoản đăng nhập", key=f"user_login_{role_can_thiet}")
            p = st.text_input("Mật khẩu", type="password", key=f"pass_login_{role_can_thiet}")
        ...
        if not user_info or (role_can_thiet and user_info.get('role') != role_can_thiet):
            st.markdown("<br><br>", unsafe_allow_html=True)
            st.error("🚫 Tài khoản của bạn không có quyền truy cập khu vực này!")
            if st.button("🔄 Thoát & Đăng nhập lại", type="primary"):
...
if st.session_state.get('current_view') == "landing_page":
    ...
    with col3:
        st.warning("### ⚙️ Cổng BGH / Quản Trị\nThống kê, xuất báo cáo và Quản lý hệ thống bản quyền.")
        if st.button("Truy Cập Quản Trị ➡️", use_container_width=True):
...
# ==========================================
    ...
    if c_btn.button("⬅️ Trở về Trang chủ"):
        st.session_state['current_view'] = "landing_page"; st.rerun()
...
# ==========================================
    ...
    if kiem_tra_dang_nhap(role_can_thiet='teacher'):
        ...
        col_menu, col_danh_sach, col_chat = st.columns([w_menu, 3.0, 6.5], gap="small")
        ...
        with col_menu:
            ...
            if st.session_state.get('menu_expanded'):
                ...
                if user_info.get('avatar'): st.markdown(f'<div style="text-align:center; margin-bottom: 20px;"><img src="data:image/png;base64,{user_info.get("avatar")}" width="45" style="border-radius: 50%;"></div>', unsafe_allow_html=True)
            ...
            if st.session_state.get('menu_gv') in ["mo", "xong"]:
                ...
                if not ma_dang_chon or ma_dang_chon not in ca_cua_toi:
                    st.markdown("""<div style='display: flex; flex-direction: column; align-items: center; justify-content: center; height: 80vh; text-align: center; color: #9CA3AF;'>
                        <h2>Chào buổi sáng! 🌻</h2><p>👉 Bấm vào một ca bất kỳ ở Hộp thư bên trái để bắt đầu.</p></div>""", unsafe_allow_html=True)
                else:
                    ca_hien_tai = ca_cua_toi[ma_dang_chon]
                    
                    c_head1, c_head2 = st.columns([5, 1])
                    c_head1.markdown(f"<h4 style='margin-bottom:0; color:{main_color};'>🗣️ {ma_dang_chon} | Lớp {ca_hien_tai.get('lop', '')} | 🎭 Cảm xúc ban đầu: {ca_hien_tai.get('cam_xuc_ban_dau', 'N/A')}</h4>", unsafe_allow_html=True)
                                    ...
                                    try:
                                        ...
                                        for key_hien_tai in keys_luot_nay:
                                            ...
                                            if res.status_code == 200:
                                                ...
                                                thanh_cong = True
                                                break  
                                        if not thanh_cong: ca_hien_tai['ai_phan_tich'] = "⏳ **LỖI:** Tài khoản AI hết hạn mức 20 lần/ngày. Vui lòng mượn Gmail khác tạo thêm API Key!"
                                        luu_du_lieu_len_may(); st.rerun()
                ...
                with c_info:
                    ...
                    st.session_state['theme_color'] = st.selectbox("", list(theme_map.keys()), index=list(theme_map.keys()).index(st.session_state.get('theme_color', 'Xanh Zalo')), label_visibility="collapsed")
